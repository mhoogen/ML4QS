...
for t in range(1, len(time)):
    pred_mood.append(pred_mood[-1] + x[-1] * (
            gamma_1 * (1 - pred_mood[-1]) * max((pred_al[-1] - pred_mood[-1]), 0) + gamma_2 * pred_mood[-1] *
            (min((pred_al[-1] - pred_mood[-1]), 0))) * delta_t)
    pred_al.append(pred_al[-1] + gamma_3 * (
            pred_al[-1] * min(math.sin(((t - (gamma_4 * math.pi)) / gamma_5)), 0) + (1 - pred_al[-1]) *
...
random_time_series.iloc[2000:3999, random_time_series.columns.get_loc('value')] = \
    random_time_series.iloc[2000:3999, random_time_series.columns.get_loc('value')] + linear_deduction[0:1999]
...
dataset['radius'] = dataset['filtered_acc_x'].pow(2) + dataset['filtered_acc_y'].pow(2) + dataset['filtered_acc_z'].pow(
    2)
...
axarr[0].set_xlabel('time')
axarr[0].set_ylabel('value')
axarr[0].legend(['$mood$', '$activity$ $level$',
                 '$predicted$ $mood$ $with$ $\gamma_{1}=\gamma_{2}=\gamma_{3}=\gamma_{4}=\gamma_{5}=1$',
                 '$predicted$ $mood$ $with$ $\gamma_{1}=\gamma_{2}=\gamma_{3}=\gamma_{4}=\gamma_{5}=1$'], loc=4,
...
plot.legend(['$pareto$ $front$', '$non-dominated$ $instance$', '$dominated$ $instance$'], loc=1, fontsize='small',
            numpoints=1)
plot.xlim([0, 1])
plot.ylim([0, 1])
plot.xlabel('$E_{X_{1}}$')

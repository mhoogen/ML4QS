...
plot.plot([0, 1], [1, 0], 'k:')
plot.legend(['$class$ $1$ $(active)$', '$class$ $2$ $(inactive)$', '$decision$ $boundary$'], loc=4, fontsize='small',
            numpoints=1)
plot.xlim([0, 1])
plot.ylim([0, 1])
plot.xlabel('$X_{1}$')
...
plot.plot(x, y_1, 'k:')
plot.plot(x, y_2, 'k:')
plot.legend(['$class$ $1$ $(active)$', '$class$ $2$ $(inactive)$', '$decision$ $boundary$'], loc=2, fontsize='small',
            numpoints=1)
...
df2['Y'] = y_2
sigma = 1
z_1 = np.power(math.e, -(sklearn.metrics.pairwise.euclidean_distances(X=df1, Y=np.array([0.5, 0.5]).
                                                                      reshape(1, -1)) / 2 * math.pow(sigma, 2)))
z_2 = np.power(math.e, -(sklearn.metrics.pairwise.euclidean_distances(X=df2, Y=np.array([0.5, 0.5]).
                                                                      reshape(1, -1)) / 2 * math.pow(sigma, 2)))
...
distances_df_full = sklearn.metrics.pairwise.euclidean_distances(X=df_full,
                                                                 Y=np.array([0.51, 0.51]).reshape(1, -1)).flatten()
ind = np.argsort(distances_df_full)[:k]
plot.plot(df_full.loc[ind, 'X'], df_full.loc[ind, 'Y'], 'y*', markersize=12)

plot.legend(['$class$ $1$ $(active)$', '$class$ $2$ $(inactive)$', '$new$ $point$', '$nearest$ $neighbors$'], loc=4,

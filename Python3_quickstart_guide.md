...
2. Create a new virtual environment using the Anaconda Navigator or the Anaconda Prompt (Windows) or terminal (MacOS/Linux). Enter `conda create --name <my_env_name> anaconda` to create a new environment, then activate it by entering `conda activate <my_env_name>`. 

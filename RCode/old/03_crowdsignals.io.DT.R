...
full_data = accelerometer[, acc_phone_x:=mean(x),by=aggWindow]
full_data = accelerometer[, acc_phone_y:=mean(y),by=aggWindow]
full_data = accelerometer[, acc_phone_z:=mean(z),by=aggWindow]
full_data[,timestamps:= {aggWindow*aggregation_window+current_time}]

fd = full_data[,list(acc_phone_x=mean(acc_phone_x),acc_phone_y=mean(acc_phone_y),
                     acc_phone_z=mean(acc_phone_z),time=mean(timestamps)),by=list(aggWindow)]
full_data=data.frame(fd[,aggWindow:=NULL])
